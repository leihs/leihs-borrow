...
;-; INIT APP & DB
(reg-event-fx ::load-app
              (fn-traced [{:keys [db]}]
                         {:db (-> db
                                  ; NOTE: clear the routing instance on
                                  ; (re-)load,
                                  ; otherwise the event wont re-run when hot
                                  ; reloading!
...
(defn main-view
  ...
  (let [errors @(subscribe [:app/fatal-errors])]
    [:main [main-nav/navbar] (when errors [ui/fatal-error-screen errors])
...
(defn- route-is-loading-view
  ...
  [:div.app-loading-view
   [:h1.text-monospace.text-center.p-8.show-after-3sec
    [:p.text-5xl [ui/spinner-clock]] [:p.font-black.text-xl "loading…"]
...
; (defn- not-found-view []
; [:h1.font-black.text-monospace.text-5xl.text-center.p-8 "404 NOT FOUND!"])
(defn- wip-models-index-view
  []
  [:h1.font-black.text-monospace.text-5xl.text-center.p-8 "WIP MODELS INDEX!"])

;-; CORE APP
(def views
  {::routes/home home-page/view,
   ...
   ; FIXME: this is used for "loading" AND "not found", find a way to
   ; distinguish.
...
(reg-event-fx ::routes/absolute-root
...
(defn mount-root
  ...
  (r/render [main-view views] (.getElementById js/document "app")))
