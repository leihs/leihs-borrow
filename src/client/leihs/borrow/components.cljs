...
(defn merge-props
  ...
  (merge defaults
         ...
         {:class (->> (get defaults :class)
                      ...
                      (map keyword)
                      set)}
         {:style (merge (get defaults :style) (get givens :style))}))
...
(defn image-square-thumb
  [image href]
  (let
    [img-src (:image-url image)
     inner
       (if img-src
         [:img.position-absolute.object-contain.object-center.h-full.w-full.p-1.bg-content
          {:src img-src}]
    ...
    [:button.border-black.border-2.rounded-full.py-1.px-3
     {:type :button,
      :on-click #(-> js/window
                     (.-location)
                     (.reload))} "RELOAD"]]
   (doall (for [[idx error] (map-indexed vector errors)]
            [:small.code {:key idx} (js/JSON.stringify (clj->js error) 0 2)]))])

(defn fatal-error-screen
  [errors]
  [:section.p-4
   {:style {:white-space "pre-wrap", :background "salmon", :padding "1rem"}}
   [:h1 "FATAL ERROR :("]
...
(defn tmp-nav
  []
  [:nav.border.border-black.m-3.p-2 [:b "NAVIGATION MENU"]
   ...
   [:p
    [:a {:href (routing/path-for ::routes/current-user-show)} "Current User"]]
...
(defn dev-nav
  []
  [:nav.border.border-black.m-3.p-2 [:b "dev nav"]
   [:p
    [:a
     {:href (routing/path-for ::routes/categories-show
                              :categories-path
                              "09ac0343-0d83-5c7f-b112-d5921e9479fd")}
     "a category show"]]
   ...
   [:p
    [:a
     {:href (routing/path-for ::routes/models-show
                              :model-id
                              "1c18b3d3-88e8-57ac-8c28-24d3f8f77604")}
     "a model show"]]])


; copied from
; <https://github.com/sindresorhus/cli-spinners/blob/af93e2f345a73a16c7686066c08dd970d66d8870/spinners.json#L720>
...
(defn spinner-clock
  ...
  (let [spinner spinner-data-clock
    ...
    (fn []
      (js/setTimeout (fn []
                       (swap! frame #(mod (inc %) (count (:frames spinner)))))
                     speed)
...
(defn format-date
  ...
  (let [date (if (= (.-constructor date) js/Date) date (js/Date. date))]
    ...
    (-> (js/Intl.DateTimeFormat. #_nil {:dateStyle style})
        (.format date))))

(def decorate-file-size-formatter
  (js/Intl.NumberFormat. :default
                         (clj->js {:maximumFractionDigits 2, :style :decimal})))

(defn decorate-file-size
  [bytes]
  (str (.format decorate-file-size-formatter
                (-> bytes
                    (/ (* 1024 1024))))
       nbsp
       "MB"))
