  ...
  (fn-traced [{:keys [_db]} [_ args {:keys [data errors]}]]
             (if errors
               ...
               {:alert (str "OK! " (pr-str data)),
                :dispatch [::reset-availability-and-fetch (:modelId args)
                           (:startDate args) (:endDate args)]})))
                ...
                (fn [e]
                  (let [val (.-value (.-target e))]
                    ...
                    (when (and (:start-date @state) (:end-date @state))
                      (dispatch [::reset-availability-and-fetch id
                                 (:start-date @state) (:end-date @state)])))))]
           ...
           [:div.d-flex.items-end.mt-2
             ...
             [:div.d-flex.flex-wrap.align-items-end
              ...
              [:div.flex-1.w-1_2
               [:span.no-underline.text-color-muted
                {:aria-label (str "maximum available quantity is "
            ...
            [:label.d-block.mb-0.mr-3 [:span.d-block.text-color-muted "for "]
             [:select
              ...
              (doall
                (for [user (cons (:user current-user)
                                 (:delegations current-user))]
                  [:option
                   {:value (:id user),
            ...
            [:div.flex-auto.w-1_2
             [:button.px-4.py-2.w-100.rounded-lg.bg-content-inverse.text-color-content-inverse.font-semibold.text-lg
              (cond-> {:on-click on-submit}
                (> (:quantity @state) (:max-quantity @state)) (assoc :disabled
                                                                true))
              "Order"]]])]))))
...
(defn view
  ...
  (let [routing @(subscribe [:routing/routing])
    ...
    [:section.mx-3.my-4
     (cond
       is-loading? [:div [:div [ui/spinner-clock]]
