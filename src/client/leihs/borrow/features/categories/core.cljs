...
(def dispatch-fetch-index-handler
  (fn-traced
    ...
    {:dispatch [::re-graph/query
                (rc/inline
                  "leihs/borrow/features/categories/getRootCategories.gql") {}
                #_{:count how-many} [::on-fetched-categories-index]]}))
...
(reg-sub ::categories-index (fn [db] (get-in db [:ls ::data])))

(defn categories-list
  [categories]
  (let [list (doall (for [category categories]
                      {:id (:id category),
    ...
    [:div.mx-1.mt-2 [:> UI/Components.CategoryList {:list list}]]))
