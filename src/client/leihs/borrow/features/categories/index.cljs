...
(reg-event-fx ::routes/categories-index categories/dispatch-fetch-index-handler)

(defn view
  []
  (fn []
    (let [cats @(subscribe [::categories/categories-index])]
      [:<> [:div.mx-4.mt-6 [:h2.font-extrabold.text-3xl "Categories"]]
       [:div.pb-8 (categories/categories-list cats)]])))
