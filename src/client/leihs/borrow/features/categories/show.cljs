...
(def query (rc/inline "leihs/borrow/features/categories/getCategoryShow.gql"))
...
(reg-event-fx
  ...
  (fn-traced
    ...
    (let [categories-path (get-in args [:route-params :categories-path])
      ...
      {:dispatch-n (list [::re-graph/query query
...
(reg-event-db
  ...
  (fn-traced [ls [_ category-id {:keys [data errors]}]]
             (-> ls
                 ...
                 (cond-> errors (assoc-in [::errors category-id] errors))
...
(reg-event-fx
  ...
  (fn-traced [_ [_ nav-args]]
             {:dispatch-n (list [::filters/clear-current]
                                ...
                                [:routing/navigate
                                 [::routes/categories-show nav-args]])}))

(reg-sub ::category-data (fn [db [_ id]] (get-in db [:ls ::data id])))
...
(defn view
  ...
  (let [routing @(subscribe [:routing/routing])
        categories-path (get-in routing
                                [:bidi-match :route-params :categories-path])
    ...
    [:<>
     (cond
       is-loading? [:div.p-4.text-center [:div.p-2 [ui/spinner-clock]]
                    [:pre "loading category" [:samp category-id] "…"]]
       errors [ui/error-view errors]
       :else
         [:<>
          [:header.mb-4.mx-3.mt-4
           [:h1.text-3xl.font-extrabold.leading-none (:name category)]
           (when parent-id
             [:span.mt-2.text-color-muted.text-sm
              [:a.text-color-content-link
               {:href (str (routing/path-for ::routes/categories-show
                                             :categories-path
                                             (join "/" prev-ids)))} "← back"]])]
               ...
               [:<> {:key id}
                [:li.d-inline-block.mb-2.mr-1
                 [:a.text-color-content.border.rounded.py-1.px-2.mb-1
                  {:class
                     "text-gray-800 bg-content border-gray-800 hover:bg-gray-200",
                   :href (-> js/window
                             .-location
                             .-pathname
                             (str "/" id))} (:name child)]]]))]
