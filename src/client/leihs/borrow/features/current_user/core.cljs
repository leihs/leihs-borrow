...
(reg-event-fx
  ...
  (fn-traced [_ [_ _]]
             {:dispatch [::re-graph/query
                         (rc/inline
                           "leihs/borrow/features/current_user/core.gql") {}
                         [::on-fetched-data]]}))
...
(reg-sub ::data (fn [db _] (get-in db [::data])))

(reg-sub ::pools :<- [::data] (fn [cu _] (:inventory-pools cu)))

(reg-sub ::suspensions :<- [::data] (fn [cu _] (:suspensions cu)))
