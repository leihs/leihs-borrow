...
(reg-event-fx
  ...
  (fn-traced
    ...
    {:dispatch
       [::re-graph/query
        (rc/inline
          "leihs/borrow/features/customer_orders/customerOrdersIndex.gql") {}
        [::on-fetched-data]]}))

(reg-event-db ::on-fetched-data
...
(defn orders-list
  [orders]
  [:ul
   (doall (for [order orders] [:li {:key (:id order)} [order-line order]]))])
  ...
  (let [data @(subscribe [::data])
    ...
    [:section.mx-3.my-4
     (cond
       ...
       :else
         [:<> [:header.mb-3 [:h1.text-3xl.font-extrabold.leading-none "Orders"]]
          (when-not (empty? submitted-orders)
            [:div.mt-3 [:h2.text-xl.font-semibold "Active Orders"]
             [orders-list submitted-orders ""]])
          (when-not (empty? rejected-orders)
            [:div.mt-3 [:h2.text-xl.font-semibold "Rejected Orders"]
             [orders-list rejected-orders]])
          (when-not (empty? approved-orders)
            [:div.mt-3 [:h2.text-xl.font-semibold "Approved Orders"]
