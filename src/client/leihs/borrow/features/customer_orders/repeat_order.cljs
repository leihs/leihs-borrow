...
(reg-event-fx
 ...
 (fn-traced [{:keys [db]} [_ rental]]
   (let [order-id (-> rental :id)
         ...
         consider-alt-pickup-locations? (->> rental :reservations
     ...
     (if (-> pool-ids count (= 1))
       {:dispatch [::re-graph/query
                   ...
                   {:id (first pool-ids)
                    ...
                    :endDate end-date
                    :considerAlternativePickupLocations consider-alt-pickup-locations?}
                   [::on-open-repeat-dialog order-id consider-alt-pickup-locations?]]
        :db (-> db (assoc-in [::data :repeat-dialog] {:loading? true}))}
       ; else
       {:db (-> db (assoc-in [::data :repeat-dialog]
                             {:show true
                              :consider-alt-pickup-locations? consider-alt-pickup-locations?}))}))))

(reg-event-db
 ::on-open-repeat-dialog
 (fn-traced [db [_ _ consider-alt-pickup-locations? {:keys [data errors]}]]
   (-> db
...
(defn repeat-dialog [rental reservations user-id date-locale text-locale]
  (let [today (date-fns/startOfToday)
        ...
        validate! (fn [start-date end-date pool consider-alt-pickup-locations?]
                    (reset!
                     validation-result
                     (cond
                       ...
                       (let [availability-messages
                             (UI/validateDateRange
                              ...
                              (boolean consider-alt-pickup-locations?))]
                         (if (seq availability-messages)
                           {:valid? false :date-messages availability-messages}
                           {:valid? true})))))
        change-selected-range (fn [r pool consider-alt-pickup-locations?]
                                (let [start-date (-> r .-startDate)
                                      end-date (-> r .-endDate)]
                                  (reset! selected-range {:startDate start-date :endDate end-date})
                                  (validate! start-date end-date pool consider-alt-pickup-locations?)))
    ...
    (fn [rental reservations user-id date-locale]
      (let [dialog-data @(subscribe [::repeat-dialog-data])
            ...
            consider-alt-pickup-locations? (-> dialog-data :consider-alt-pickup-locations?)
        ...
        (if-not (-> dialog-data :show)
          ...
          [:> UI/Components.Design.ModalDialog
           ...
           [:> UI/Components.Design.ModalDialog.Body
            [:form {:on-submit (fn [e]
                                 (-> e .preventDefault)
                                 (validate! (:startDate @selected-range) (:endDate @selected-range) pool consider-alt-pickup-locations?)
                   ...
                   [UiDateRangePicker
                    (merge calendar-availability-props
                           {:locale date-locale
                            ...
                            :onChange #(change-selected-range % pool consider-alt-pickup-locations?)
                            :min-date today
                            :max-date max-date})]
