...
(reg-event-fx
 ...
 (fn-traced [{:keys [db]} [_ rental]]
   (let [reservations (:reservations rental)
     ...
     (if (-> pool-ids count (= 1))
       {:dispatch [::re-graph/query
        ...
        :db (assoc-in db [::data :repeat-dialog] {:loading? true})}
       {:db (assoc-in db [::data :repeat-dialog] {:show true})}))))

(reg-event-fx
 ::on-open-repeat-dialog
 (fn-traced [{:keys [db]} [_ {:keys [data errors]}]]
   (if errors
     {:db (dissoc-in db [::data :repeat-dialog])
      :dispatch [::errors/add-many errors]}
     {:db (assoc-in db [::data :repeat-dialog] {:show true
                                                :pool (:inventory-pool data)})})))
