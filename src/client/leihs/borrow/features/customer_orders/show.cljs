  ...
  (fn-traced
    ...
    (let [order-id (get-in args [:route-params :order-id])]
      {:dispatch
         [::re-graph/query
          (rc/inline
            "leihs/borrow/features/customer_orders/customerOrderShow.gql")
          {:id order-id} [::on-fetched-data order-id]]})))
...
(reg-sub ::data (fn [db [_ id]] (get-in db [::data id])))

(reg-sub ::errors (fn [db _] (::errors db)))

(defn reservation-line
  [quantity line]
  (let [model (:model line)
    ...
    [:div.flex.flex-row.border-b.mb-2.pb-2.-mx-1
     ...
     [:div.px-1.flex-1 [:div.font-semibold (:name model)]
      [:div.text-sm (ui/format-date :short (get-in line [:start-date]))
       (str ui/thin-space "–" ui/thin-space)
       (ui/format-date :short (get-in line [:end-date]))]
      [:div.text-sm.text-color-muted [:span quantity " Items"] [:span " • "]
       [:span pool]]]
     #_[:div.px-1.self-center.flex-none
        [:button.text-sm {:on-click #(js/alert "TODO")} ui/trash-icon]]]))
    ...
    [:section.mx-3.my-4
     (cond
       ...
       :else
         [:<>
          [:header.mb-3
           [:h1.text-3xl.font-extrabold.leading-tight "Order “" (:purpose order)
            "”"] [:p "from: " (ui/format-date :short (:created-at order))]
           [:p.mt-2.text-color-muted.text-sm
            [:a {:href (routing/path-for ::routes/orders-index)}
             "← all Orders"]]] #_[:pre "?" (get-in order [:sub-orders-by-pool])]
          #_(doall (for [suborder (get-in order [:sub-orders-by-pool])]
                     [:div "suborder" [:p (pr-str suborder)]
                      (doall (for [r (:reservations suborder)]
                               [reservation-line r]))]))
          [:h2.font-bold.text-xl [:mark "Order Data"]]
          [:pre.text-xs {:style {:white-space :pre-wrap}}
           (js/JSON.stringify (clj->js order) 0 2)] #_[:p]])]))
