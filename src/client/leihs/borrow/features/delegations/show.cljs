  ...
  (fn-traced
    ...
    (let [delegation-id (get-in args [:route-params :delegation-id])]
      {:dispatch [::re-graph/query
                  ...
                  {:id delegation-id} [::on-fetched-data delegation-id]]})))
...
(reg-event-db
  ...
  (fn-traced [db [_ delegation-id {:keys [data errors]}]]
             (-> db
                 ...
                 (cond-> errors (assoc-in [::errors delegation-id] errors))
                 (assoc-in [::data delegation-id] (:delegation data)))))

(reg-sub ::delegation (fn [db [_ id]] (get-in db [::data id])))

(reg-sub ::errors (fn [db [_ id]] (get-in db [::errors id])))

(defn fullname [user] (str (:firstname user) " " (:lastname user)))

(defn responsible
  [user]
  [:<> [:h3 "Responsible:"] [:div (fullname user)]
   (if-let [email (:email user)] [:a {:href (str "mailto:" email)} email])])
...
(defn view
  ...
  (let [routing @(subscribe [:routing/routing])
        delegation-id (get-in routing
                              [:bidi-match :route-params :delegation-id])
    ...
    [:section.mx-3.my-4
     (cond
       ...
       :else [:<>
              ...
              [:br] [responsible (:responsible delegation)] [:br]
