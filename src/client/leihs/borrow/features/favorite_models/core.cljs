...
;-; EVENTS
(reg-event-fx ::routes/models-favorites
              (fn-traced [_ _] {:dispatch [::models/get-models EXTRA-PARAMS]}))
...
(defn view
  ...
  (let [models @(subscribe [::models/data])]
    [:<>
     [:header.mx-3.my-4 [:h1.text-3xl.font-extrabold.leading-none "Favorites"]]
     [models/search-and-list
      #(dispatch [:routing/navigate
                  [::routes/models-favorites {:query-params %}]])
      #(dispatch [::clear]) EXTRA-PARAMS]]))
