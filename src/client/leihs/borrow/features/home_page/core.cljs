...
(reg-event-fx
  ::routes/home
  (fn-traced
    [_ [_ {:keys [query-params]}]]
    {:dispatch-n (-> (list [::categories/fetch-index 4])
                     (cond-> (seq query-params) (conj [::filters/set-multiple
                                                       query-params]))
                     (conj [::filters/init]))}))
    ...
    (let [cats @(subscribe [::categories/categories-index])]
      [:<>
       [models/search-panel
        ...
        #(dispatch [::models/clear])] [:hr]
       [:div
        [:div.mt-2.mx-3.d-flex.align-items-baseline.justify-content-between
         [:h2.font-bold.text-2xl "Categories"]
         [:a.font-semibold.text-l
          {:href (routing/path-for ::routes/categories-index)} "All"]]
        (categories/categories-list (take 4 cats))] [:hr]])))
