(ns leihs.borrow.features.model-show.core
  ...
  (:require
    ...
    [leihs.borrow.lib.re-frame :refer
     [reg-event-fx reg-event-db reg-sub #_reg-fx subscribe dispatch
      camel-case-keys]]
  ...
  (fn-traced
    ...
    (let [now (js/Date.)
          start-date (datefn/startOfMonth (or start-date now))
            ...
            (datefn/endOfMonth
              (cond
                (and end-date (<= start-date end-date)) end-date
                :else (datefn/addMonths now 3)))]
      {:dispatch
         [::re-graph/query
...
(reg-event-fx
  ...
  (fn-traced
    [{:keys [db]} [_ model-id start-date end-date]]
    {:db
       (update-in
         db
         [:ls ::data model-id]
...
(reg-event-db
  ...
  [(path :ls)]
  (fn-traced
    [ls [_ model-id {:keys [data errors]}]]
    (->
      ls
...
(reg-sub ::errors (fn [db [_ id]] (get-in db [:ls ::errors id])))
...
(reg-event-fx
  ::on-mutation-result
  (fn-traced
    [{:keys [_db]} [_ args {:keys [data errors]}]]
    (if errors
      {:alert (str "FAIL! " (pr-str errors))}
      {:alert (str "OK! " (pr-str data)),
       :dispatch
         [::reset-availability-and-fetch (:modelId args) (:startDate args)
          (:endDate args)]})))
...
(defn order-panel
  ...
  (let [filter-start (:start-date filters)
        ...
        start-date (datefn/startOfMonth (or filter-start now))
          ...
          (datefn/endOfMonth
            (cond
              (and filter-end (<= start-date filter-end)) filter-end
              :else (datefn/addMonths now 3)))
      ...
      (when has-availability?
        [:> UI/Components.BookingCalendar
         {:initialOpen true,
          ...
          ; END DYNAMIC FETCHING PROPS
          :onLoadMoreFuture
            #(js/alert (str "Load More!" (js/JSON.stringify %))),
          :onSubmit on-submit,
...
(defn order-panel-1
  ...
  (let [state
    ...
    (fn [{:keys [id], :as model} _]
      (swap! state assoc
        :max-quantity
        (:available-quantity-in-date-range model))
                ...
                (fn [e]
                  (let [val (.-value (.-target e))]
                    ...
                    (when (and (:start-date @state) (:end-date @state))
                      (dispatch
                        [::reset-availability-and-fetch id (:start-date @state)
                         (:end-date @state)])))))]
        [:div.border-b-2.border-gray-300.mt-4.pb-4
         [:h3.font-bold.text-lg.Xtext-color-muted.mb-2 "Make a reservation"]
         [:div.d-flex.mx-n2
               ...
               [:span.no-underline.text-color-muted
                {:aria-label
                   (str
                     ...
                     (:max-quantity @state))} "/" ui/thin-space
                (or (:max-quantity @state) [ui/spinner-clock]) ui/thin-space
                "max."]]]]
            [:label.d-block.mb-0.mr-3 [:span.d-block.text-color-muted "for "]
             [:select
              {:name :user-id,
               :on-change
                 (fn [e]
              ...
              (doall
                (for [user
                        (cons (:user current-user) (:delegations current-user))]
                  [:option
                   {:value (:id user),
                    :selected (= (:user-id @state) (:id user))}
                   (if (= (:user-id @state) (:id user)) "me" (:name user))]))]]
            [:div.flex-auto.w-1_2
     ...
     (cond
       is-loading?
         [:div [:div [ui/spinner-clock]]
          [:pre "loading model" [:samp model-id] "…"]]
       errors [ui/error-view errors]
       :else
         [:<>
          [:header.d-flex.items-stretch
           [:h1.text-3xl.font-extrabold.leading-none (:name model) " "
            [:small.font-normal.text-gray-600.leading-none
             (:manufacturer model)]]
          ...
          [order-panel model filters] #_[order-panel-1 model filters]
          ...
          (if-let [recommends
                     (->
                       ...
                       :edges
                       not-empty)]
            [:div.mt-4 [:h2.text-xl.font-bold "Ergänzende Modelle"]
