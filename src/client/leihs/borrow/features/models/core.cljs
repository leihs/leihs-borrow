...
(def query-gql (rc/inline "leihs/borrow/features/models/getModels.gql"))

;-; EVENTS
(reg-event-fx ::routes/models
              (fn-traced [_ [_ {:keys [query-params]}]]
                         {:dispatch-n (list [::filters/set-multiple
                                             query-params]
                                            [::get-models])}))
...
(reg-event-fx
  ::get-models
  (fn-traced
    [{:keys [db]} [_ extra-args]]
    (let [filters (filters/current db)
          start-date (:start-date filters)
          end-date (:end-date filters)
          dates-valid? (<= start-date end-date) ; if somehow end is before
                                                ; start, ignore it instead of
                                                ; error
          query-vars
            (merge
      ...
      {:db (assoc-in db [::data] nil),
       :dispatch [::re-graph/query query-gql query-vars
                  [::on-fetched-models]]})))

(reg-event-fx
  ::on-fetched-models
  (fn-traced
    [{:keys [db]} [_ {:keys [data errors]}]]
    (if errors
...
(reg-event-db ::clear-data (fn-traced [db _] (dissoc db ::data)))


(reg-sub ::fetching (fn [db _] (get-in db [::data :fetching])))
...
(reg-sub ::data
         (fn [db]
           (-> (get-in db [::data :edges]))))

;-; VIEWS
(defn form-line
  [name label input-props]
  [:label.row [:span.text-xs.col-3.col-form-label (str label " ")]
   [:div.col-9
    [:input
     (merge input-props
            {:name name,
...
(defn search-panel
  ...
  (let [filters @(subscribe [::filters/current])
        ...
        on-search-view? (= (get-in routing [:bidi-match :handler])
                           ::routes/models)]
    [:div.px-3.py-4.bg-light
     {:class (when on-search-view? "mb-4"),
     ...
     [:form.form.form-compact
      ...
      [:div.form-group
       [form-line :search-term "Suche"
        {:type :text,
         :value term,
         :on-change #(dispatch [::filters/set-one :term
                                (-> %
                                    .-target
                                    .-value)])}]]
      [:div.form-group
       [form-line :user-id "Für"
        {:type :text, :disabled true, :value (or (:user-id filters) "")}]]
      [:div.form-group
       [:div.row [:span.text-xs.col-3.col-form-label (str "Zeitraum" " ")]
        [:div.col-9
         [:label.custom-control.custom-checkbox
          [:input.custom-control-input
           {:type :checkbox,
            ...
            :on-change (fn [_]
                         (dispatch [::filters/set-one :available-between?
       ...
       [:div.form-group
        [form-line :start-date "von"
         {:type :date,
          ...
          :value start-date,
          :on-change #(dispatch [::filters/set-one :start-date
                                 (-> %
                                     .-target
                                     .-value)])}]]
       [:div.form-group
        [form-line :end-date "bis"
         {:type :date,
          ...
          :min start-date,
          :value end-date,
          :on-change #(dispatch [::filters/set-one :end-date
                                 (-> %
                                     .-target
                                     .-value)])}]]
       [:div.form-group
        [form-line :quantity "Anzahl"
         {:type :number,
          :min 1,
          :value quantity,
          :on-change #(dispatch [::filters/set-one :quantity
                                 (-> %
                                     .-target
                                     .-value)])}]]]
      [:button.btn.btn-success.dont-invert.rounded-pill
       {:type :submit, :class :mt-2} "Get Results"]
                   ...
                   (let [model (:node m)
                     ...
                     {:id (:id model),
                      ...
                      :href (routing/path-for ::routes/models-show
                                              :model-id
                                              (:id model))})))]
    [:div.mx-1.mt-2 [:> UI/Components.CategoryList {:list models-list}]
...
(defn load-more
  ...
  (let [fetching-more? @(subscribe [::fetching])
        ...
        dates-valid? (<= start-date end-date) ; if somehow end is before start,
                                              ; ignore it instead of error
       ...
       (when has-next-page?
         [:div.p-3.text-center
          [:button.border.border-black.p-2.rounded
           {:on-click #(dispatch
                          [::pagination/get-more query-gql
                           (merge {:searchTerm term,
                                   :startDate start-date,
                                   :endDate end-date,
                                   :onlyAvailable only-available?,
                                   :bothDatesGiven (boolean (and dates-valid?
                                                                 start-date
                                                                 end-date))}
                                  extra-args) [::data] [:models]])}
           "LOAD MORE"]]))]))
...
(defn search-and-list
  ...
  (let [models @(subscribe [::data])]
    [:<> [search-panel submit-fn clear-fn]
     (cond (nil? models) [:p.p-6.w-full.text-center.text-xl [ui/spinner-clock]]
           (empty? models) [:p.p-6.w-full.text-center "nothing found!"]
           :else [:<> [models-list models] [load-more extra-params]])]))
