...
(reg-event-fx ::routes/pools-index
              (fn-traced [_ _] {:dispatch [::current-user/fetch]}))

(defn pool-line [pool] [:<> (:name pool) [badge pool]])
...
(defn view
  ...
  (let [pools @(subscribe [::current-user/pools])
    ...
    [:section.mx-3.my-4
     (cond is-loading? [:div
                        [:div.text-center.text-5xl.show-after-1sec
                         [ui/spinner-clock]]]
           ; errors [ui/error-view errors]
           :else
             [:<>
              [:header.mb-3 [:h1.text-3xl.font-extrabold.leading-none "Pools"]]
              (when-not (empty? pools) [:div.mt-3 [pools-list pools ""]])])]))
