  ...
  (fn-traced [_ [_ args]]
             (let [pool-id (get-in args [:route-params :pool-id])]
               {:dispatch [::re-graph/query
                           ...
                           {:id pool-id} [::on-fetched-data pool-id]]})))
...
(reg-event-db
  ...
  (fn-traced [db [_ pool-id {:keys [data errors]}]]
             (-> db
                 ...
                 (cond-> errors (assoc-in [:ls ::errors pool-id] errors))
                 (assoc-in [:ls ::data pool-id] (:inventory-pool data)))))

(reg-sub ::pool (fn [db [_ id]] (get-in db [:ls ::data id])))
...
(defn view
  ...
  (let [routing @(subscribe [:routing/routing])
    ...
    [:section.mx-3.my-4
     (cond is-loading? [:div [:div [ui/spinner-clock]]
                        [:pre "loading pool" [:samp (:id pool)] "…"]]
