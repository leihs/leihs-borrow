...
(reg-event-fx
  ...
  (fn-traced
    ...
    {:dispatch [::re-graph/query
                (rc/inline
                  "leihs/borrow/features/shopping_cart/getShoppingCart.gql") {}
                [::on-fetched-data]]}))

(reg-event-db
  ::on-fetched-data
  (fn-traced
    [db [_ {:keys [data errors]}]]
    (-> db
        (assoc ::data (help/kebabize-keys
                        (get-in data [:current-user :unsubmitted-order])))
...
(reg-event-fx ::reset-availability-and-fetch
              (fn-traced
                [{:keys [db]} [_ args]]
                {:db (update-in db [::data :edit-mode] dissoc :max-quantity),
...
(reg-event-fx
  ...
  (fn-traced
    ...
    {:dispatch
       [::re-graph/query
        (rc/inline
          "leihs/borrow/features/shopping_cart/getModelAvailableQuantity.gql")
        args [::on-fetched-available-quantity]]}))
...
(reg-event-fx
  ...
  (fn-traced
    ...
    {:dispatch
       [::re-graph/mutate
        (rc/inline
          "leihs/borrow/features/shopping_cart/deleteReservationLines.gql")
        {:ids ids} [::on-delete-reservations]]}))
...
(reg-event-fx
  ...
  (fn-traced
    ...
    {:dispatch [::re-graph/mutate
                (rc/inline
                  "leihs/borrow/features/shopping_cart/submitOrderMutation.gql")
                args [::on-submit-order-result]]}))

(reg-event-fx ::on-submit-order-result
...
(reg-event-fx
  ...
  (fn-traced
    ...
    {:dispatch [::re-graph/mutate
                (rc/inline
                  "leihs/borrow/features/shopping_cart/updateReservations.gql")
                args [::on-update-reservations-result]]}))

(reg-event-fx ::on-update-reservations-result
              (fn-traced [{:keys [db]}
                          [_
                           {:keys [errors],
                            {del-ids :delete-reservation-lines,
                             new-res-lines :create-reservation}
                              :data}]]
                         (if errors
                           {:alert (str "FAIL! " (pr-str errors))}
                           {:db (update-in db
...
(reg-sub ::edit-mode-data :<- [::data] (fn [data _] (:edit-mode data)))

(reg-sub ::max-quantity :<- [::edit-mode-data] (fn [emd _] (:max-quantity emd)))
...
(reg-sub ::reservations :<- [::data] (fn [co _] (:reservations co)))
...
(reg-sub ::reservations-grouped
         ...
         (fn [lines _]
           (->> lines
                (group-by (fn [line] [(get-in line [:model :id])
...
(reg-sub ::order-summary
         ...
         [::reservations]
         (fn [rs _]
           {:pools (-> (map :inventory-pool rs)
                       distinct),
            :total-models (-> (map :model rs)
                              distinct
                              count),
            :total-items (-> (map :model rs)
                             count),
            :earliest-start-date (-> (map :start-date rs)
                                     sort
                                     first),
            :latest-end-date (-> (map :end-date rs)
                                 sort
                                 last)}))
...
(defn edit-reservation
  ...
  (let [edit-mode-data @(subscribe [::edit-mode-data])
            ...
            (fn [e]
              (let [v (.-value (.-target e))]
                ...
                (when (and @start-date @end-date)
                  (dispatch [::reset-availability-and-fetch (:id model)
                             start-date end-date])))))
    ...
    [:div.border-b-2.border-gray-300.mt-4.pb-4
     ...
     [:div.d-flex.mx-n2
      [:label.px-2.w-1_2 [:span.text-color-muted "from "]
       [:input
        {:type :date,
         :name :start-date,
         :value start-date,
         :on-change (fn [e]
                      (let [new-start-date (-> e
                                               .-target
                                               .-value)]
                        ...
                        (dispatch
                          [::reset-availability-and-fetch
                           {:modelId (:id model),
                            ...
                            :endDate end-date,
                            :excludeReservationIds (map :id res-lines)}])))}]]
      [:label.px-2.w-1_2 [:span.text-color-muted "until "]
       [:input
        {:type :date,
         :name :end-date,
         :value end-date,
     ...
     [:div.d-flex.items-end.mt-2
      [:div.w-1_2
       [:div.d-flex.flex-wrap.align-items-end
        [:div.w-1_2
         [:label.d-block.mb-0 [:span.d-block.text-color-muted "quantity "]
          [:input.w-full
           {:type :number,
            :name :quantity,
            :max max-quantity,
        ...
        [:div.flex-1.w-1_2
         [:span.no-underline.text-color-muted
          {:aria-label (str "maximum available quantity is " max-quantity)} "/"
          ui/thin-space (or max-quantity [ui/spinner-clock]) ui/thin-space
          "max."]]]]
      [:div.w-1_2
       [:label.d-block.mb-0 [:span.d-block.text-color-muted "for"]
        [:input.w-full
         {:type :text, :name :user-id, :disabled true, :value user-id}]]]
      [:div.flex-auto.w-1_2
       [:button.px-4.py-2.w-100.rounded-lg.font-semibold.text-lg
        {:on-click #(dispatch [::cancel-edit])} "Cancel"]
       [:button.px-4.py-2.w-100.rounded-lg.bg-content-inverse.text-color-content-inverse.font-semibold.text-lg
        (cond-> {:on-click on-update}
          (> quantity max-quantity) (assoc :disabled true)) "Update"]]]]))

;_; VIEWS
(defn reservation
  [res-lines invalid-res-ids]
  (let [exemplar (first res-lines)
        model (:model exemplar)
     ...
     (if edit-mode?
       ...
       [:div
        [:div.px-1.flex-1 (if invalid? {:style {:color "red"}})
         ...
         [:div.text-sm (ui/format-date :short (get-in exemplar [:start-date]))
         ...
         [:div.text-sm.text-color-muted (if invalid? {:style {:color "red"}})
          [:span quantity " Items"] [:span " • "] [:span pool-names]]]
        ...
        [:div.px-1.self-center.flex-none
         ...
         [:button.rounded.border.border-gray-600.px-2.text-color-muted
          {:on-click #(dispatch [::edit-reservation res-lines])} "Edit"]]])]))
  ...
  (let [state (reagent/atom {:purpose ""})]
    (fn []
      (let [data @(subscribe [::data])
        ...
        [:div.p-2 [:h1.mt-3.font-bold.text-3xl "Order Overview"]
         (cond
           ...
           (empty? grouped-reservations)
             [:div.bg-content-muted.text-center.my-4.px-2.py-4.rounded-lg
              ...
              [:a.d-inline-block.text-xl.bg-content-inverse.text-color-content-inverse.rounded-pill.px-4.py-2.my-4
               {:href (routing/path-for ::routes/home)} "Borrow Items"]]
           :else
             [:<>
              [:div
               [:div.mt-2.mb-4.flex
                ...
                [:div.flex-none.px-1
                 [:button.rounded.border.border-gray-600.px-2.text-color-muted
                  "edit"]]]
               (doall (for [[grouped-key res-lines] grouped-reservations]
                        [:<> {:key grouped-key}
                         [reservation res-lines invalid-res-ids]]))
               [:div.mt-4.text-sm.text-color-muted
                [:p "Total " (:total-models summary) ui/nbsp "Model(s), "
                 (:total-items summary) ui/nbsp "Item(s), " "from "
               ...
               [:div
                ...
                [:button.w-100.p-2.my-4.rounded-full.bg-content-danger.text-color-content-inverse.text-xl
                 {:on-click #(dispatch [::delete-reservations
                                        (map :id reservations)])}
                 "Delete order"]]]])]))))
