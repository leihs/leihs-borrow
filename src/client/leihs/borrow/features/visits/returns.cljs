(ns leihs.borrow.features.visits.returns
  (:require
...
(reg-event-fx
  ::routes/returns-index
  (fn-traced [{:keys [db]} [_ _]]
    {:dispatch [::re-graph/query
                (rc/inline "leihs/borrow/features/visits/returns.gql")
                {}
                [::on-fetched-data]]}))
  ...
  (let [data @(subscribe [::data])
    ...
    [:section.mx-3.my-4
     (cond
       ...
       [:<>
        [:header.mb-3
         [:h1.text-3xl.font-extrabold.leading-tight (t :returns/title)]]
