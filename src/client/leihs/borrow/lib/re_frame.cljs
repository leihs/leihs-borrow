...
(defn- kebab-case-keys
  [m]
  (postwalk #(cond-> %
               (and (keyword? %) (not (qualified-keyword? %))) csk/->kebab-case)
            m))
...
(def base-interceptors
  [localstorage-interceptor kebab-case-data-and-errors-interceptor])

(defn reg-event-db
  ([event-id event-handler]
   (rf/reg-event-db event-id base-interceptors event-handler))
...
(defn reg-event-fx
  ([event-id event-handler]
   (rf/reg-event-fx event-id base-interceptors event-handler))
  ([event-id interceptors event-handler]
