;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; based on
; <https://github.com/MattiNieminen/re-fill/blob/960b4eaf/src/re_fill/routing.cljs>
; ;
    ...
    (let [pushy-instance (get-in @db/app-db [:routing/routing :pushy-instance])]
      (if-not pushy-instance
        (let [dispatch-fn #(dispatch [:routing/change-view %])
              match-fn (fn [path]
                         (let [is-routed? (:handler (bidi/match-route routes
                                                                      path))]
...
(reg-event-fx :routing/init-routing
...
(reg-fx :routing/navigate-raw
        (fn [url] (aset (.-location js/window) "href" url)))

(reg-event-fx :routing/navigate-raw
              (fn-traced [_ [_ url]] {:routing/navigate-raw url}))

(reg-event-fx :routing/refresh-page (fn-traced [_] {:routing/refresh-page nil}))

(reg-fx :routing/refresh-page (fn [_] (.reload (.-location js/window))))

(reg-event-fx ::scroll-to-top (fn-traced [_] {::scroll-to-top nil}))
(reg-fx ::scroll-to-top (fn [_] (js/window.scrollTo 0 0)))

(reg-sub :routing/routing (fn [db _] (:routing/routing db)))
...
(defn bidi-path-for-with-query-params
  [routes-map name & args]
  (if-not (= (namespace name) (namespace ::routes/ns))
    (throw (js/Error. (str "route name is from wrong namespace! "
                           (pr-str name)))))
...
; TODO: define this next to routes (so this lib does not need to import
; routes!)
(defn path-for
  [name & args]
  (apply bidi-path-for-with-query-params routes/routes-map name args))
