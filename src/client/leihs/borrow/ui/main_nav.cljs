...
(defn navbar
  ...
  (let [cart-data @(subscribe [:leihs.borrow.features.shopping-cart.core/data])
        ...
        ; partials
        toggler [:a.nav-item.nav-link.px-0
                 {:href (routing/path-for ::routes/about-page)} icons/menu-icon]
        brand [:a.navbar-brand.m-0.font-black.text-xl
               {:href (routing/path-for ::routes/home)} (str " " "LEIHS" " ")]
        valid-until [:span
                     (if @(subscribe [::cart/timed-out?])
                       {:style {:color "red"}}
                       {:class "text-color-info"}) (:valid-until cart-data)]
        cart [:a.nav-item.nav-link.px-0
    ...
    [:nav.ui-main-nav.navbar.navbar-light.text-xl.shadow-md.py-0.px-2.sticky-top.flex-nowrap.justify-content-between
     {:style style} [:div.navbar-nav.w-100 [:div.mr-auto toggler]]
     [:div.mx-auto brand]
     [:div.navbar-nav.w-100
      [:div.ml-auto.d-flex.align-items-center
       [:div.mx-auto.px-2.text-xs valid-until] " " cart]]]))
