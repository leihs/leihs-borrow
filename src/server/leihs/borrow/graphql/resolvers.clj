(ns leihs.borrow.graphql.resolvers
  (:require [clojure.tools.logging :as log]
            ...
            [leihs.core.graphql.helpers :refer
             [transform-resolvers wrap-resolver-with-error
              wrap-resolver-with-kebab-case wrap-resolver-with-camelCase]]
            [java-time :refer [local-date before?]])
  (:import (java.time.format DateTimeFormatter)))

(defn wrap-resolver-with-dates-validation
  [resolver]
  (fn [context {:keys [start-date end-date], :as args} value]
    (if (and start-date
             end-date
             (before? (local-date DateTimeFormatter/ISO_LOCAL_DATE end-date)
