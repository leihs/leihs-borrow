...
(comment (->> {:user-id "c0777d74-668b-5e01-abb5-f8277baa0ea8"}
...
(defn get-roots
  [{{:keys [tx], {user-id :id} :authenticated-entity} :request} {:keys [limit]}
   _]
  (-> {:user-id user-id, :limit (cond->> (str limit) limit (str "LIMIT "))}
      all-reservable-root-categories-sqlvec
      (->> (jdbc/query tx))))
...
(defn get-one
  [{{:keys [tx]} :request} {:keys [id parent-id]} _]
  (-> base-sqlmap
      (cond-> parent-id (-> (sql/select :model_groups.id
                                        [(sql/call :coalesce
                            ...
                            (sql/merge-where [:= :model_group_links.parent_id
                                              parent-id])))
