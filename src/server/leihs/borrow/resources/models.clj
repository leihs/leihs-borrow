(ns leihs.borrow.resources.models
  (:require [clojure.spec.alpha :as spec]
            ...
            [leihs.borrow.resources.inventory-pools.visits-restrictions :as
             restrict]
...
(def base-sqlmap
  (-> (sql/select
        :models.*
        [(sql/raw
           "trim(both ' ' from concat_ws(' ', models.product, models.version))")
         :name])
...
(defn merge-reservable-conditions
  [sqlmap user-id]
  (-> sqlmap
      (sql/merge-join :items [:= :models.id :items.model_id])
      ...
      (sql/merge-join
        (sql/raw (str "(" entitlements/all-sql ") AS pwg"))
        [:and [:= :models.id :pwg.model_id]
         [:= :inventory_pools.id :pwg.inventory_pool_id] [:> :pwg.quantity 0]
         [:or
          [:in :pwg.entitlement_group_id
...
(defn reservable?
  [{{:keys [tx], {user-id :id} :authenticated-entity} :request, :as context} _
   value]
  (-> base-sqlmap
...
(defn merge-search-conditions
  ...
  (sql/merge-where sqlmap
                   ["~~*"
                    (sql/call :concat_ws
                              " " :models.product
                              :models.version :models.manufacturer)
                    (str "%" search-term "%")]))

(defn merge-category-ids-conditions
  [sqlmap category-ids]
  (-> sqlmap
      (sql/merge-join :model_links [:= :models.id :model_links.model_id])
      (sql/merge-where [:in :model_links.model_group_id category-ids])))

(defn get-category-ids
  [tx category-id direct-only]
  (if direct-only
    [category-id]
    (as-> category-id <>
...
(defn available-quantity-in-date-range
  ...
  [{{tx :tx, {user-id :id} :authenticated-entity} :request, :as context}
   {:keys [inventory-pool-ids start-date end-date exclude-reservation-ids],
    :as args} value]
  (or
    (:available-quantity-in-date-range value)
    (if (before? (local-date DateTimeFormatter/ISO_LOCAL_DATE start-date)
                 (local-date))
      0
      (let [pool-ids
              (or (not-empty inventory-pool-ids)
                  (map :id
...
(defn merge-available-quantities
  [models {{tx :tx, {user-id :id} :authenticated-entity} :request, :as context}
   {:keys [start-date end-date]} value]
  (map (if (before? (local-date DateTimeFormatter/ISO_LOCAL_DATE start-date)
                    (local-date))
...
(defn get-availability
  [{{tx :tx, {user-id :id} :authenticated-entity} :request, :as context}
   {:keys [start-date end-date inventory-pool-ids]} value]
  (let [pool-ids (or inventory-pool-ids
                     (map :id
                       (pools/to-reserve-from tx user-id start-date end-date)))]
    (map
      (fn [pool-id]
        (let [pool (pools/get-by-id (-> context
                                        :request
                                        :tx)
                                    pool-id)
              avail (availability/get context
                                      {:start-date start-date,
                                       :end-date end-date,
...
(defn from-compatibles
  [sqlmap value user-id unscope-reservable]
  (-> sqlmap
      (sql/from :models_compatibles)
      (cond-> (not unscope-reservable) (merge-reservable-conditions user-id))
      (sql/join :models [:= :models.id :models_compatibles.compatible_id])
      (sql/merge-where [:= :models_compatibles.model_id (:id value)])))

(defn merge-categories-conditions
  [sqlmap tx category-id direct-only]
  (let [category-ids (get-category-ids tx category-id direct-only)]
    (cond-> sqlmap
      (seq category-ids) (merge-category-ids-conditions category-ids))))

(defn get-multiple-sqlmap
  [{{:keys [tx authenticated-entity]} :request, :as context}
   {:keys [ids category-id limit offset direct-only order-by search-term
           unscope-reservable is-favorited]} value]
  (->
    base-sqlmap
    (cond-> (= (::lacinia/container-type-name context) :Model)
              (from-compatibles value
                                ...
                                unscope-reservable))
    (cond-> (or category-id
                (= (::lacinia/container-type-name context) :Category))
              (merge-categories-conditions tx
                                           (or category-id (:id value))
                                           direct-only))
    (cond-> (not unscope-reservable) (merge-reservable-conditions
                                       (:id authenticated-entity)))
    (cond-> (seq ids) (sql/merge-where [:in :models.id ids]))
    (cond-> search-term (merge-search-conditions search-term))
    (cond-> (not (nil? is-favorited))
              (-> (sql/merge-left-join :favorite_models
                                       [:= :favorite_models.model_id
                                        :models.id])
                  (sql/merge-where [(if is-favorited :!= :=)
                                    :favorite_models.model_id nil])))
    (cond-> (seq order-by) (-> (sql/order-by (helpers/treat-order-arg order-by))
                               (sql/merge-order-by [:name :asc])))
    (cond-> limit (sql/limit limit))
    (cond-> offset (sql/offset offset))))

(defn get-favorites-sqlmap
  [{{{user-id :id} :authenticated-entity} :request, :as context} args value]
  (-> (get-multiple-sqlmap context (assoc args :unscope-reservable true) value)
      (sql/merge-join :favorite_models
                      [:and [:= :models.id :favorite_models.model_id]
                       [:= :favorite_models.user_id user-id]])))

(defn favorited?
  [{{:keys [tx], {user-id :id} :authenticated-entity} :request} _ value]
  (-> (sql/select (sql/call :exists
                            (-> (sql/select true)
                                (sql/from :favorite_models)
                                (sql/where [:and [:= :model_id (:id value)]
                                            [:= :user_id user-id]]))))
      sql/format
      (->> (jdbc/query tx))
      first
  ...
  (let [field-args (some->>
    ...
    (cond-> models
      (and (executor/selects-field? context :Model/availableQuantityInDateRange)
           (:start-date field-args) ; checking this because `selects-field?`
                                    ; does not respect @include directive
           (:end-date field-args)) ; checking this because `selects-field?` does
                                   ; not respect @include directive
