  ...
  (let [pools (pools/get-multiple context {:ids inventory-pool-ids} nil)]
    (map (fn [{pool-id :id}]
           (let [pool (pools/get-by-id (-> context :request :tx)
                 ...
                 alt-locations (when (pools/enable-alternative-pickup-locations? tx pool-id)
                                 (pickup-locations/get-by-pool-id tx pool-id))
                 first-alt-location-id (some-> alt-locations first :id)
      ...
      (cond-> (seq order-by)
        (-> (dissoc :order-by)
            (as-> sqlmap
                  (apply sql/order-by
                         sqlmap
                         (helpers/treat-order-arg order-by :models)))))
      (cond-> limit
        (sql/limit limit))
      (cond-> offset
        (sql/offset offset))))
