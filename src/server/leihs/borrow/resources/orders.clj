...
(defn multiple-base-sqlmap
  ...
  (let [common-columns [:id :purpose :created_at :updated_at]]
    (-> (sql/select :orders_union.id
        ...
        (sql/from [{:union [(-> (apply sql/select common-columns)
                   ...
                   :orders_union])
        (sql/join :orders
                  [:or [:= :orders_union.id :orders.customer_order_id]
                   [:= :orders_union.id :orders.id]])
        (assoc :group-by [:orders_union.id :orders_union.purpose
                          :orders_union.created_at :orders_union.updated_at]))))

(defn pool-order-row [row] (update row :state upper-case))

(defn get-one
  [{{:keys [tx], {user-id :id} :authenticated-entity} :request} {:keys [id]} _]
  (-> (sql/select :*)
...
(defn equal-condition [a1 a2] [:and ["@>" a1 a2] ["<@" a1 a2]])

(defn get-connection-sql-map
  [{{:keys [tx], {user-id :id} :authenticated-entity} :request}
   {:keys [order-by states]} value]
  (->
    (multiple-base-sqlmap tx user-id)
    (cond-> states (sql/having (equal-condition
...
(defn get-multiple-by-pool
  [{{:keys [tx]} :request, :as context} {:keys [order-by]} value]
  (let [columns (as-> (database/columns tx "orders") <>
    ...
    (->
      ...
      ; sub-order too.
      (sql/where [:or [:= :customer_order_id (:id value)] [:= :id (:id value)]])
      (cond-> (seq order-by) (sql/order-by (helpers/treat-order-arg order-by)))
      sql/format
      (as-> <> (jdbc/query tx <> {:row-fn pool-order-row})))))
  ...
  (let [rs (-> (reservations/unsubmitted-sqlmap tx user-id)
    ...
    {:valid-until (valid-until tx user-id),
     ...
     :invalidReservationIds (->> rs
                                 (reservations/with-invalid-availability
                                   context)
                                 (map :id))}))
        ...
        (if (seq group-el)
          (let [order (-> (sql/insert-into :orders)
            ...
            (-> (sql/update :reservations)
                (sql/set
                  {:status (sql/call :cast "submitted" :reservation_status),
                   :inventory_pool_id pool-id,
                   :order_id (:id order)})
                ...
                sql/format
                (reservations/query tx))
            (recur remainder (conj mails #(mails/send-received context order))))
          (do (set! ds/after-tx mails) customer-order))))))
