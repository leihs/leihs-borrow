...
(defn query
  ...
  (jdbc/query tx
              sql-format
              {:row-fn #(update % :status clojure.string/upper-case)}))
...
(defn unsubmitted-sqlmap
  [tx user-id]
  (-> (apply sql/select (columns tx))
      (sql/from :reservations)
      (sql/where
        [:and [:= :status (sql/call :cast "unsubmitted" :reservation_status)]
         [:= :user_id user-id]])))

(defn for-customer-order
  [tx user-id]
...
(defn with-invalid-availability
  ...
  (->> reservations
       ...
       (group-by
         #(-> %
              (select-keys [:model_id :inventory_pool_id :start_date :end_date])
              vals))
       (reduce
         (fn [invalid-rs [[model-id pool-id start-date end-date] rs]]
           (let [available-quantity (models/available-quantity-in-date-range
                                      context
                                      {:inventory-pool-ids [pool-id],
                                       :start-date start-date,
                                       :end-date end-date,
                                       :exclude-reservation-ids (map :id
                                                                  reservations)}
                                      {:id model-id})]
             (cond-> invalid-rs
               (< available-quantity (clojure.core/count rs)) (into rs))))
         [])))
...
(defn valid-until-sql
  [tx]
  [(sql/call :to_char
             (sql/raw (str "updated_at + interval '"
                           (:timeout_minutes (settings/get tx))
                           " minutes'"))
             helpers/date-time-format) :updated_at])

(defn touch-unsubmitted!
  [tx user-id]
...
(defn delete
  [{{:keys [tx], {user-id :id} :authenticated-entity} :request} {:keys [ids]} _]
...
(defn create
  ...
  (let [pools (cond->> (pools/to-reserve-from tx user-id start-date end-date)
                       (seq inventory-pool-ids)
                       (filter #((set inventory-pool-ids) (:id %))))]
    (if (empty? pools)
      (throw (ex-info
               "Not possible to reserve from any pool under given conditions."
               {}))
      (let [pool-avails
